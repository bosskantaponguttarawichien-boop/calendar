...
export default function ResultScreen() {
  const stats = [
    { label: "กิจกรรมทั้งหมด", value: "12", icon: Calendar, color: "text-blue-500", bg: "bg-blue-50 dark:bg-blue-900/20" },
    { label: "เสร็จสิ้นแล้ว", value: "8", icon: CheckCircle2, color: "text-green-500", bg: "bg-green-50 dark:bg-green-900/20" },
    { label: "รอดำเนินการ", value: "4", icon: Clock, color: "text-orange-500", bg: "bg-orange-50 dark:bg-orange-900/20" },
  ];
  ...
  return (
    <div className="flex-grow overflow-y-auto px-1 pb-4">
        ...
        {recentActivities.map((activity, i) => (
          <button
            ...
            className={`w-full flex items-center justify-between px-5 py-4 active:bg-slate-50 dark:active:bg-slate-700/50 transition-colors ${
              i < recentActivities.length - 1 ? "border-b border-slate-100/50 dark:border-slate-700/50" : ""
            }`}
          >
            <div className="flex items-center gap-4 text-left">
              <div className={`w-2 h-2 rounded-full ${activity.status === "เสร็จสิ้น" ? "bg-green-400 dark:bg-green-500" : "bg-orange-400 dark:bg-orange-500"}`} />
              <div>
      ...
      <div className="bg-gradient-to-br from-slate-800 to-slate-900 dark:from-slate-900 dark:to-black border dark:border-slate-800 rounded-3xl p-6 text-white shadow-lg overflow-hidden relative">
